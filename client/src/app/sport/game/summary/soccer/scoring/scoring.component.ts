...
@Component({
  ...
  styleUrls: ["./scoring.component.css","../summary-soccer.component.css"],
})
export class ScoringComponent implements OnInit {
  ...
  public dataSource: PlayerScoringDataSource;
  public displayedColumns = ["name", "scoringG","scoringA","scoringPTS",];
  ...
  public ngOnInit() {
    const player: any[] = this.data
      .map((x) => {
        return {
          ...
          scoringG : x.G,
          scoringA:x.A,
          scoringPTS:(x.G + x.A)*2
        };
      });
    let totalCol: any = {};
    totalCol["name"] = "TOTAL";
    totalCol["scoringG"] = player.length === 0 ? "-" : player.reduce((sum, item) => sum + (item.scoringG ? item.scoringG : 0), 0);
    totalCol["scoringA"] = player.length === 0 ? "-" : player.reduce((sum, item) => sum + (item.scoringA ? item.scoringA : 0), 0);
    totalCol["scoringPTS"] = player.length === 0 ? "-" : player.reduce((sum, item) => sum + (item.scoringPTS ? item.scoringPTS : 0), 0);
    ...
    this.dataSource = new PlayerScoringDataSource(player, this.sort);
  }
...
export class PlayerScoringDataSource extends DataSource<any> {
