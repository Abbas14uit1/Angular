...
const allPlayersRoute = express_1.Router();
exports.allPlayersRoute = allPlayersRoute;
/**
 * Get all players, optionally filtering by season
 */
allPlayersRoute.get("/:sportCode/", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (req.query.season && !season) {
        ...
        try {
            const players = yield db.collection("players").aggregate(pipeline).toArray();
            res.status(200).json({
                data: players,
            });
        }
        ...
        const players = yield db.collection("players").find({ sportCode: { $eq: req.params.sportCode } }).toArray();
        res.status(200).json({
            data: players,
        });
    }
 ...
 */
allPlayersRoute.get("/:playerId/playerStats", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
    try {
        console.log(req.params.playerId);
        const player = yield db.collection("players").findOne({ _id: req.params.playerId });
        res.status(200).json({
            data: player,
        });
    }
...
allPlayersRoute.get("/:playerId/aggregate", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
...
allPlayersRoute.get("/:playerId/superlatives", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
    const from = req.query.from ? moment(req.query.from, "YYYY-MM-DD").toDate() : undefined;
    const to = req.query.to ? moment(req.query.to, "YYYY-MM-DD").toDate() : undefined;
    const season = req.query.season ? Number.parseInt(req.query.season) : undefined;
    ...
    try {
        ...
        for (const game of player.games) {
            ...
            sups.push({
                gameId: game.gameId,
                superlatives: yield db.collection("superlatives").find({ _id: { $in: game.superlatives } }).toArray(),
            });
        }
        res.status(200).json({
            data: sups,
...
allPlayersRoute.get("/:playerId/games", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
    const from = req.query.from ? moment(req.query.from, "YYYY-MM-DD").toDate() : undefined;
    const to = req.query.to ? moment(req.query.to, "YYYY-MM-DD").toDate() : undefined;
    const season = req.query.season ? Number.parseInt(req.query.season) : undefined;
    ...
    try {
        ...
        const games = yield db.collection("games").find({ _id: { $in: gameIds } }).toArray();
        res.status(200).json({
            data: games,
        });
    }
...
allPlayersRoute.get("/:sportCode/roasters/:teamId/:season", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
    const seasonNumber = Number(req.params.season);
    let playerPosition = req.params.sportCode === "MFB" ?
...
allPlayersRoute.get("/seasons/:sportCode/:teamId", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    const db = req.app.locals.db;
...
allPlayersRoute.put("/", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    console.log("Update");
    const db = req.app.locals.db;
    const existingPlayer = yield db.collection("players").findOne({ _id: req.body.id });
    if (existingPlayer) {
        try {
            yield db.collection("players").updateOne({ "_id": req.body.id }, {
                $set: {
                    name: req.body.name,
...
//# sourceMappingURL=allPlayers.route.js.map