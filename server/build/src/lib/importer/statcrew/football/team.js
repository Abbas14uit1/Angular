 ...
 */
class Team extends StatcrewImportBase_1.StatcrewImportBase {
    constructor() {
        super();
        this.dependents = [];
    ...
    updateDependents() {
        for (const dependent of this.dependents) {
            ...
            else if (dependent instanceof teamGame_1.TeamGame) {
                dependent.updateTeamRef(this.getId());
            }
            else {
     ...
     */
    parse(inputTeam, opponent, venue, quarterTotals, gameDate, actualDate) {
        if (quarterTotals === undefined) {
            quarterTotals = [{}];
            quarterTotals[0].qtrsummary = [{}, {}];
        }
        if (gameDate === undefined || actualDate === undefined)
            return;
        this.parsedData = teamParser_1.parseTeam(inputTeam, opponent, quarterTotals, gameDate, actualDate, venue);
    }
    /**
