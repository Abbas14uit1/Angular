 ...
 */
class Player extends StatcrewImportBase_1.StatcrewImportBase {
    constructor() {
        super();
        this.playerClass = ["FR", "SO", "JR", "SR"];
        this.dependents = [];
     ...
     */
    parse(player, teamCode, teamName, teamConference, teamConferenceDivision, teamTidyName, opponentCode, opponentName, opponentConference, opponentConferenceDivision, side, gameDate, actualDate) {
        this.parsedData = playerParser_1.parsePlayer(player, teamCode, teamName, teamConference, teamConferenceDivision, teamTidyName, opponentCode, opponentName, opponentConference, opponentConferenceDivision, side, gameDate, actualDate);
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            ...
            if (this.parsedData.playerId != null) {
                ...
                const result_update = yield db.collection("players").updateOne({ playerId: this.parsedData.playerId }, { "$set": { playerId: this.parsedData.playerId,
                        name: this.parsedData.name,
                        sportCode: "MFB",
                        teamCode: this.parsedData.teamCode,
                        teamId: this.parsedData.teamId,
                ...
                const result = yield db.collection("players").findOne({ playerId: this.parsedData.playerId });
                if (result) {
                    id = result._id;
                }
            }
                ...
                const matchedPlayers = yield query.toArray();
                for (const player of matchedPlayers) {
                    const samePlayer = yield this.areTheySamePlayers(db, player._id, player);
                    if (samePlayer === true) {
                        id = player._id;
     ...
     */
    getQueryString(minAcceptedSeason, maxAcceptedSeason) {
        return [
            {
                $match: {
                    $and: [
     ...
     */
    areTheySamePlayers(db, id, player) {
        return __awaiter(this, void 0, void 0, function* () {
            const currentSeason = this.parsedData.games[0].season;
            ...
            if (currentSeason >= player.minSeason && currentSeason <= player.maxSeason) {
                ...
                const resultCode = yield this.matchPlayerByIdAndPlayerClass(db, id);
                if (resultCode > 0) {
                    return true;
                }
                else {
            ...
            if (currentSeason < player.minSeason) {
                ...
                const resultCode = yield this.matchPlayerCareerWithPlayerClassBackward(db, id);
                ...
                const resultCode = yield this.matchPlayerCareerWithPlayerClassForward(db, id);
     ...
     * @param id Player id
     */
    matchPlayerCareerWithPlayerClassBackward(db, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const playerDetailBySeason = yield this.getPlayerClassBySeason(db, id);
            let result = 0;
            let startIndex = -1;
            ...
            const redShirtedSeason = this.getPlayerRedShirtedSeason(playerDetailBySeason);
            ...
            const indexToMove = startIndex - currentStartIndex;
     ...
     * @param id Player id
     */
    matchPlayerCareerWithPlayerClassForward(db, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const playerDetailBySeason = yield this.getPlayerClassBySeason(db, id);
            let result = 0;
            let startIndex = -1;
     ...
     */
    matchPlayerByIdAndPlayerClass(db, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const playerDetailBySeason = yield this.getPlayerClassBySeason(db, id);
            let result = 0;
            let count = 0;
            ...
            const numberOfSeasonsPlayed = playerDetailBySeason.length;
    ...
    getPlayerClassBySeason(db, id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const query = yield db.collection("players").aggregate(queryString);
            const playerDetailBySeason = yield query.toArray();
     ...
     */
    getPlayerRedShirtedSeason(matchedPlayers) {
        let previousClass = "";
        let result = 0;
        for (const player of matchedPlayers) {
