...
import * as PlayerParser from "../../../../../src/lib/importer/statcrew/helpers/playerParser";
...
const gameDate: Date = new Date(); // current date to satisfy parser

test("parse single player from file, skipping stats", (t) => {
  ...
  player.parse(statcrewPlayer, 8, "Alabama", "Southeastern Conference",
  "East", "UA", 657, "USC", "Pac-12 Conference", "North",
  VH.home, moment("9/3/2016", "MM-DD-YYYY").toDate(), "9/3/2016");
  t.not(player, undefined);
  t.is(player.parsedData.games[0].side, VH.home);
...
test("parse DEFENSE stats tack total", (t) => {
  const stats = {
    $: {
      tackua: 2,
      tacka: 1,
...
test("parse DEFENSE stats tack total should not be overwritten", (t) => {
  const stats = {
    $: {
      tackua: 2,
      tacka: 1,
...
test("parse DEFENSE stats sack total", (t) => {
  const stats = {
    $: {
      tackua: 2,
      tacka: 1,
...
test("parse DEFENSE stats sack total should not be overwritten", (t) => {
  const stats = {
    $: {
      tackua: 2,
      tacka: 1,
...
test("parse IR (interception return) stats", (t) => {
  const stats = {
    $: {
      no: 1,
      yds: 0,
...
test("parse PR (punt return) stats", (t) => {
  const stats = {
    $: {
      no: 2,
      yds: 4,
