...
import { parsePlays, Play } from "../../../../../src/lib/importer/statcrew/baseball/play";
...
import { readJson } from "../_data.helperBaseball";

let simpleParsed: AthlytePlay.IPlay;
let data: IStatcrewBaseballJSON;

test.before("create single play excerpt", (t) => {
  const play = new Play();
  const singlePlay: StatcrewPlay.IPlay = {
...
test("parsing entire file throws no errors", async (t) => {
  t.notThrows(() => parsePlays(data.plays[0],"MBA"));
});

test("parsing entire file produces list of plays", async (t) => {
  t.true(parsePlays(data.plays[0],"MBA").length > 0);
});

test("inning: parses inning number", (t) => {
...
test("scoring: parses play with outs set to zero", (t) => {
  t.is(simpleParsed.outs, 0);
});

test("out: check if the outs are counted", (t) => {
  const singlePlay: StatcrewPlay.IPlay = {
...
test("check for faulty possession before save.", (t) => {
  const singlePlay: StatcrewPlay.IPlay = {
    $: {"seq":71,"outs":1,"batter":"O'Connor","batprof":"R","pitcher":"Stripling","pchprof":"R"},"batter":[{"$":{"name":"O'Connor","code": "H1" ,"action":"KS","out":1,"adv":0,"tobase":0,"ab":1,"k":1}}],"pitcher":[{"$":{"name":"Stripling", "code": "V2","bf":1,"ip":1,"ab":1,"k":1}}],"pitches":[{"$":{"text":"KBFS","b":1,"s":3}}],"fielder":[{"$":{"pos":"c","name":"Gonzalez", "code": "V3","po":1}}],"narrative":[{"$":{"text":"O'Connor struck out swinging (1-2 KBFS)."}}],
  };
  const play = new Play();  
