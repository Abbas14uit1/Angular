...
import * as AthlyteGame from "../../../../../../../../typings/athlyte/baseball/game.d";
...
export function parseTeam(
  inputTeam: StatcrewTeam.ITeam, opponent: StatcrewTeam.ITeam, gameDate: Date, actualDate: string, sportCode: "MBA" | "WSB",
...
/**
   ...
   */
export function parseTeamInfo(teams: StatcrewTeam.ITeam[]) {
 ...
 * @param team team information from statcrew JSON
 */
export function parseStarters(team: StatcrewTeam.ITeam) {
  const starters: AthlyteTeam.IStarter[] = [];
...
export function parseTotals(team: StatcrewTeam.ITeam) {
  ...
  totals.hitting = {
      ...
      tb: computeTotalBases(team.totals[0].hitting[0].$.h || 0,
    ...
    totals.pitching = {
      ...
      tb: computeTotalBases(team.totals[0].pitching[0].$.h || 0,
                  team.totals[0].pitching[0].$.double || 0, 
                  team.totals[0].pitching[0].$.triple || 0,
                  team.totals[0].pitching[0].$.hr || 0),
    } ;
    let hitSummary: CommonStats.IHitSummary | undefined;
    ...
    let pitchSummary: CommonStats.IPitchSummary | undefined;
...
function parseLineScore(team: StatcrewTeam.ITeam): AthlyteTeam.ITeamLineScore {
  ...
  const parsedInningss: AthlyteTeam.ITeamInningScore[] = [];
 ...
 * @param team team information from statcrew JSON
 */
export function parseRecords(team: StatcrewTeam.ITeam) {
  ...
  function computeTotalBases(oneb: number, twob: number, threeb: number, hr: number): number {
