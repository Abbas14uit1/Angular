 ...
 * to saving to Mongo.
 */
export class Game extends BaseballStatcrewImportBase {
  ...
  public parseMeta(venue: StatcrewVenue.IVenue): AthlyteGame.IGameMeta {
    ...
    const officials: StatcrewVenue.IVenueUmpires = venue.umpires[0].$;
    const venueRules: StatcrewVenue.IVenueRules = venue.rules[0].$;
    const parsedOfficials: AthlyteGame.IUmpireInfo[] = [];
  ...
  public parse(
    ...
    plays: StatcrewPlay.IAllPlaysSummary[],
  ): void {
...
export function parseSummary(
  plays: StatcrewPlay.IAllPlaysSummary[],
  teams: StatcrewTeam.ITeam[],
  longThresh = 70,
): AthlyteGame.IGameSummary {
  const scores: number[] = [];
  const played: boolean[] = [];
  const battingDetails: AthlyteGame.IBattingDetails[] = [];
  const driveStartStop: number[][] = [];
  const fgas: number[] = [];
  const longplay: number[] = [];
  const driveIndex = 0;
  const teamNames: StatcrewTeam.IMetaTeamStats[] = []; //Note: index 0 is always home team
  ...
  if (teams[0].linescore[0].lineinn !== undefined) {
    ...
    for( const inning of plays[0].inning){    
      let battingDetail: AthlyteGame.IBattingDetails;
      
      battingDetails.push({
        team: inning.batting[0].$.vh == 'H'?VH.home:VH.visitor,
      ...
      battingDetails.push({
        team: inning.batting[1].$.vh == 'H'?VH.home:VH.visitor,
