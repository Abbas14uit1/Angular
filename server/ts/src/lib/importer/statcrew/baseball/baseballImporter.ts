...
import { IPlayer } from "../../../../../../../typings/athlyte/baseball/player.d";
import { IPlayer as StatcrewPlayer } from "../../../../../../../typings/statcrew/baseball/team.d";
/**
 ...
 */
export class BaseballImporter extends AthlyteImporter {
  private game: Game;
  private plays: Play[];
  private sportCode: "MBA" | "WSB";
  private players: {
    homePlayers: Player[],
   ...
   */
  public parse(input: IStatcrewBaseballJSON): void {
    if (input.venue[0].$.sportcode != "MBA" && input.venue[0].$.sportcode != "WSB"){
      winstonLogger.log("info",input.venue[0].$.sportcode);
      throw new Error("Sport code not found. File not pre processed. Sport code: " + input.venue[0].$.sportcode );     
    }
    ...
    this.game.parse(input.venue[0], input.team, input.plays);
  }
...
export async function saveGameComponents(
  ...
  homeTeam: Team, visitingTeam: Team, plays: Play[], homeTeamGame: TeamGame, visitingTeamGame: TeamGame
) {
  ...
  try {
    ...
    const visitorGameId = await visitingTeamGame.save(db);
...
export function registerAllDependents(
  ...
  homePlayers: Player[], visitorPlayers: Player[], plays: Play[], homeTeamGame: TeamGame, visitingTeamGame: TeamGame
): void {
  // games
