...
export function parsePlayer(
  ...
  side: VH,
  gameDate: Date,
  actualDate: string, sportCode: "MBB" | "WBB"):
  AthlytePlayer.IPlayer {

  let stats: AthlyteCommon.ICommonStats | undefined;
  let prdStats1: AthlyteCommon.ICommonStats | undefined;
  let prdStats2: AthlyteCommon.ICommonStats | undefined;
...
function computeTripleDoubles(tp: number, stl: number, blk: number, ast: number, treb: number): number {
  let count = 0
  if (tp >= 10) {
    count = count + 1
  }
...
function computeDoubleDoubles(tp: number, stl: number, blk: number, ast: number, treb: number): number {
  let count = 0
  if (tp >= 10) {
    count = count + 1
  }
