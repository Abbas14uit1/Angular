...
export function constructDescription(
  ...
  type: string){
	...
	interface MapofString {[s:string]: StringMap};

	let action_description: MapofString = {
	...
	let [firstname, lastname]: string[] = player.split(" ");
	...
	const playerName: string = firstname + " " + lastname;
